...
# extract most significant digit as major version
		ncc_ax_$2_version_string="$1";
		ncc_ax_$2_version_major=`expr ${ncc_ax_$2_version_string} : '\([[0-9]]*\)'`;
# extract next most significant digit as minor version
		ncc_ax_$2_version_minor=`expr ${ncc_ax_$2_version_string} : '[[0-9]]*\.\([[0-9]]*\)'`;

# extract least significant digit as micro version
		ncc_ax_$2_version_micro=`expr ${ncc_ax_$2_version_string} : '[[0-9]]*\.[[0-9]]*\.\([[0-9]]*\)'`;
		AS_IF([test "x$ncc_ax_$2_version_micro" = "x"], [ncc_ax_$2_version_micro="0";])

# calculate a cannonical version number
		ncc_ax_$2_version_number=`expr $ncc_ax_$2_version_major \* 10000 \+  $ncc_ax_$2_version_minor \* 1000 \+ $ncc_ax_$2_version_micro \* 100`;
#---------------------------------------------------------------
...
#---------------------------------------------------------------
			...
			AC_MSG_CHECKING(["for astyle version >= ${ncc_ax_astyle_desired_version_number}"])
			...
			AS_IF([test "x${ASTYLE_TOOL}" = "x"],[],[
				AS_IF([test ${ncc_ax_astyle_actual_version_number} -ge ${ncc_ax_astyle_desired_version_number}],
					[ncc_ax_astyle_ok=yes]) ])

			dnl export variables for use within makefile
			AS_IF([test "x${ncc_ax_astyle_ok}" = "xyes"],
	...
	AM_CONDITIONAL([HAVE_ASTYLE],
			[test "x${ncc_ax_astyle_ok}" = "xyes"])

	AS_IF([test "x${ncc_ax_astyle_ok}" = "xyes"],
			[AC_MSG_RESULT(["found suitable version ${ncc_ax_astyle_actual_version_number}"])],
			[AC_MSG_RESULT(["found obsolete version ${ncc_ax_astyle_actual_version_number} but it was too old"])])
#---------------------------------------------------------------
